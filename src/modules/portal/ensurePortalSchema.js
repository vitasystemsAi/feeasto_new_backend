...
async function ensurePortalSchema() {
  ...
  if (roleCol[0] && !String(roleCol[0].Type).includes("CUSTOMER_ADMIN")) {
    await pool.query(`
  ...
  if (!(await columnExists("restaurants", "is_active"))) {
    await pool.query("ALTER TABLE restaurants ADD COLUMN is_active TINYINT(1) NOT NULL DEFAULT 1 AFTER approval_status");
  }
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS advertisements (
      ...
      ad_type ENUM('HOMEPAGE_BANNER','CAROUSEL_BANNER','RESTAURANT_SPONSORED','FOOD_SPONSORED','POPUP') NOT NULL,
  ...
  if (!(await columnExists("feedback", "deleted_at"))) {
    await pool.query("ALTER TABLE feedback ADD COLUMN deleted_at DATETIME NULL");
  }
  if (!(await columnExists("feedback", "updated_by_user_id"))) {
    await pool.query("ALTER TABLE feedback ADD COLUMN updated_by_user_id BIGINT NULL");
  }
  if (!(await columnExists("review_moderation", "is_visible"))) {
    await pool.query("ALTER TABLE review_moderation ADD COLUMN is_visible TINYINT(1) NOT NULL DEFAULT 1");
  }
  if (!(await columnExists("restaurant_priorities", "is_active"))) {
    await pool.query(
      "ALTER TABLE restaurant_priorities ADD COLUMN is_active TINYINT(1) NOT NULL DEFAULT 1"
    );
  }
  if (!(await columnExists("advertisements", "impression_count"))) {
    await pool.query("ALTER TABLE advertisements ADD COLUMN impression_count INT NOT NULL DEFAULT 0");
  }
  if (!(await columnExists("advertisements", "click_count"))) {
    await pool.query("ALTER TABLE advertisements ADD COLUMN click_count INT NOT NULL DEFAULT 0");
  }
}
